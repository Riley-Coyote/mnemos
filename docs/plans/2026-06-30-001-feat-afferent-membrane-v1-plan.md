...
| Identity-domain dream content is preempted at the gate | planned `tests/test_dream_graduation.py::test_non_identity_stabilized_pattern_emits_proposal` proves non-identity stabilized patterns emit proposals | planned `tests/test_dream_graduation.py::test_identity_domain_dream_content_preempts_stabilization_and_decays_audit_only` proves a pattern whose graduating content classifies identity/foundational at the narrative-gate domain check transitions directly `recurring` → `decayed` (`drop:high_blast_generated`); no proposal row, no private write, no stabilized-then-drop deadlock |
