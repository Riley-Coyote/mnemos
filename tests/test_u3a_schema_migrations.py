...
def test_migration_version_guards(tmp_path):
    ...
    try:
        # Bootstrap empty DB to the current latest schema.
        assert run_migrations(empty, target_version=SCHEMA_VERSION) == [4, 5, 6]
...
def test_failed_migration_rolls_back_and_preserves_schema_version(tmp_path):
    """v6 inner-life ledger exists as a real migration now; use slot 7 to test
    failure isolation — the DB rolls back to v6 (current latest) when a
    hypothetical v7 migration fails partway through.
    """
