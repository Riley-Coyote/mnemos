...
Current branch state: U1 packet mode split and U2 proposal-ledger/read-visibility schema are implemented on `codex/afferent-membrane-v1-ledger`. Historical Stage 2 note: `U3 through U6b remain planned follow-up stages`; U5 and U6b have since advanced in the implementation updates below. U6c (dream graduation, Amendment B) is scheduled as a subsequent stage per its dependency line (AM-U6 live + accumulated dream corpus).

Implementation update, 2026-07-05: U5 inert DynamicModulation storage is
implemented on `u5-inert-storage` as schema v10 plus store lifecycle helpers:
`EngramStore.store_dynamic_modulation()`,
`get_dynamic_modulation()`, `count_dynamic_modulations()`, and
`delete_dynamic_modulations_by_rollout_tag()`. It intentionally did not add
`mnemos/affect/*`, retrieval/salience influence, MCP tools, or an active read
path; those remain future U6 activation work under the same RFC-R7/R8 boundary.

Implementation update, 2026-07-05: U6b proposal-only ExperienceTick is
implemented on `u6b-experience-tick` in
`mnemos/modulation/experience_tick.py` and exported from `mnemos.modulation`.
It emits `review_only` pending ProposalLedger rows targeting the U5 modulation
surface (`MODULATION_PROPOSAL_SURFACE`) and never writes or reads
`dynamic_modulations` rows. Its focused proof is
`tests/test_u6b_experience_tick.py`: rollout-tag/TTL validation,
identity/foundational-domain refusal, per-family kill-switch behavior,
deterministic re-tick IDs, review visibility, finite valence/decay validation,
and all-or-nothing rollback for validation and store-level failures.
...
| RFC-R7 | Dynamic modulation double-bounded | U5/U6, extended by U6c (dream graduation) | U5: `mnemos/store/sqlite_store.py::EngramStore.store_dynamic_modulation`, `mnemos/store/sqlite_store.py::EngramStore.count_dynamic_modulations`, `mnemos/store/sqlite_store.py::EngramStore.delete_dynamic_modulations_by_rollout_tag`, `mnemos/store/migrations.py::migrate_v10_dynamic_modulation_storage`; U6: planned `mnemos/affect/dynamic_modulation.py`, planned `mnemos/affect/valence_floor.py`, `mnemos/retrieval/reactive.py`, planned `mnemos/dream_patterns/pattern_store.py`, planned `mnemos/substrate/handlers/dreaming.py`, planned `mnemos/substrate/handlers/wandering.py` | U5 inert-storage tests in `tests/test_u5_inert_modulation.py`; U6 valence-floor tests planned; dream-pattern adaptive-decay and non-accumulating-graduation tests planned |
| RFC-R8 | Salience fast-track, authority-qualified | U5/U6b/U6, extended by U6c (dream graduation) | U5: `mnemos/store/sqlite_store.py::EngramStore.store_dynamic_modulation`, `mnemos/store/sqlite_store.py::write_proposal`; U6b actual: `mnemos/modulation/experience_tick.py::ExperienceTick`; U6 planned: planned `mnemos/affect/dynamic_modulation.py`, `mnemos/retrieval/reactive.py`, planned `mnemos/inner_life/narrative_gate.py`, planned `mnemos/dream_patterns/pattern_store.py` | U5 modulation/proposal storage tests proving salience cannot mint semantic truth by persistence alone; U6b actual `tests/test_u6b_experience_tick.py`; active influence and four-condition-conjunctive-gate tests remain planned |
...
| RFC-R3 | U2, extended by U6c (dream graduation) | `mnemos/store/sqlite_store.py::write_proposal`, `mnemos/store/sqlite_store.py::list_proposals`, `mnemos/store/sqlite_store.py::get_proposal`, `mnemos/store/migrations.py`, `mnemos/mcp_server.py::mnemos_review_queue`, `mnemos/mcp_server.py::mnemos_proposal_audit`, planned `mnemos/inner_life/low_stakes.py::graduate_dream_pattern`, planned `mnemos/dream_patterns/pattern_store.py::emit_graduation_proposal` | `tests/test_store.py::TestEngramStore::test_proposal_ledger_accepts_rfc_authority_and_kind_axes`; `tests/test_mcp_surface.py::test_review_queue_includes_review_only_proposal_rows_with_provenance`; planned `tests/test_dream_graduation.py::test_graduation_emits_ledger_row`; planned `tests/test_dream_graduation.py::test_producer_dream_graduation_is_stamped_and_immutable` | `tests/test_store.py::TestEngramStore::test_proposal_upsert_rejects_rejected_terminal_conflict`; `tests/test_store.py::TestEngramStore::test_proposal_upsert_rejects_applied_terminal_conflict`; `tests/test_mcp_surface.py::test_review_queue_excludes_terminal_review_only_proposals`; planned `tests/test_dream_graduation.py::test_producer_dream_graduation_rows_are_filtered_from_operational_surfaces_by_explicit_producer_check` |
| RFC-R4 | U4 (evidence-diversity definition per AM-U4 subsection) | planned `mnemos/review/gates.py::classify_review_requirement`, planned `mnemos/review/gates.py::apply_review_decision`, `mnemos/store/sqlite_store.py::write_proposal`, planned `mnemos/store/sqlite_store.py::apply_proposal_decision`, planned `mnemos/mcp_server.py::mnemos_proposal_decide`, planned `mnemos/review/evidence_diversity.py::compute_evidence_diversity`, planned `mnemos/review/consistency_anomaly.py::compute_consistency_anomaly`, planned `mnemos/store/proposal_audit_log.py` (append-only per-decision audit records) | planned `tests/test_afferent_review_gates.py::test_non_foundational_hypomnema_auto_applies_after_authority_checks`; planned `tests/test_afferent_review_gates.py::test_user_stated_foundational_identity_applies_only_after_david_review`; planned `tests/test_afferent_review_gates.py::test_evidence_diverse_support_qualifies_when_min_pairwise_dissimilarity_clears_threshold`; planned `tests/test_afferent_review_gates.py::test_consistency_anomaly_surfaces_without_decision_threshold`; planned `tests/test_afferent_review_gates.py::test_mixed_set_qualifies_under_dedupe_then_diversity`; planned `tests/test_afferent_review_gates.py::test_reviewed_decision_api_transitions_deferred_to_applied_appending_audit_record`; planned `tests/test_afferent_review_gates.py::test_reviewed_decision_api_transitions_deferred_to_rejected_appending_audit_record` | planned `tests/test_afferent_review_gates.py::test_foundational_revision_without_review_is_blocked`; `tests/test_store.py::TestEngramStore::test_proposal_ledger_defaults_to_audit_only_and_rejects_pending_operational_visibility`; planned `tests/test_afferent_review_gates.py::test_paraphrase_variant_set_does_not_qualify_as_evidence_diverse`; planned `tests/test_afferent_review_gates.py::test_single_instance_or_incomputable_diversity_routes_to_review`; planned `tests/test_store.py::TestEngramStore::test_raw_write_proposal_still_raises_on_deferred_row_mutation_proposal_terminal_statuses_unchanged` |
...
| RFC-R7 | U5/U6, extended by U6c (dream graduation) | U5 actual: `mnemos/store/sqlite_store.py::EngramStore.store_dynamic_modulation`, `mnemos/store/sqlite_store.py::EngramStore.get_dynamic_modulation`, `mnemos/store/sqlite_store.py::EngramStore.count_dynamic_modulations`, `mnemos/store/sqlite_store.py::EngramStore.delete_dynamic_modulations_by_rollout_tag`, `mnemos/store/migrations.py::migrate_v10_dynamic_modulation_storage`; U6 planned: planned `mnemos/affect/dynamic_modulation.py::store_modulation`, planned `mnemos/affect/dynamic_modulation.py::bounded_retrieval_adjustment`, planned `mnemos/affect/valence_floor.py::apply_valence_floor`, `mnemos/retrieval/reactive.py::ReactiveRetriever.retrieve`, planned `mnemos/substrate/handlers/dreaming.py::apply_dream_pattern_match`, planned `mnemos/substrate/handlers/wandering.py::route_dream_output`, planned `mnemos/dream_patterns/pattern_store.py::adaptive_decay` | U5 actual `tests/test_u5_inert_modulation.py::test_u5_store_and_load_roundtrip`; U5 actual `tests/test_u5_inert_modulation.py::test_u5_g2_requires_positive_ttl`; legacy planned `tests/test_dynamic_modulation.py::test_modulation_decays_to_baseline_within_ttl` maps to stored `ttl_seconds`/`expires_at` proof only; planned `tests/test_dynamic_modulation_valence_floor.py::test_opposite_valence_same_topic_items_remain_above_floor`; planned `tests/test_dream_graduation.py::test_adaptive_decay_transitions_unrevisited_patterns_to_decayed_state_audit_only_retained` | U5 actual `tests/test_u5_inert_modulation.py::test_u5_read_surfaces_byte_identical_with_and_without_modulations`; U5 actual `tests/test_u5_inert_modulation.py::test_u5_no_module_reads_modulation_table_outside_sanctioned_files`; planned `tests/test_dynamic_modulation.py::test_dynamic_modulation_cannot_be_cited_as_evidence`; planned `tests/test_dynamic_modulation.py::test_dynamic_modulation_cannot_promote_by_recurrence`; planned `tests/test_dynamic_modulation_valence_floor.py::test_deadband_items_fail_toward_width`; planned `tests/test_dream_graduation.py::test_graduation_is_non_accumulating`; planned `tests/test_dream_graduation.py::test_single_stabilized_pattern_emits_exactly_one_proposal` |
| RFC-R8 | U5/U6b/U6, extended by U6c (dream graduation) | U5 actual: `mnemos/store/sqlite_store.py::EngramStore.store_dynamic_modulation`, `mnemos/store/sqlite_store.py::write_proposal`; U6b actual: `mnemos/modulation/experience_tick.py::ExperienceTick`, `mnemos/modulation/experience_tick.py::ProposedModulation`; U6 planned: planned `mnemos/affect/dynamic_modulation.py::store_modulation`, `mnemos/retrieval/reactive.py::ReactiveRetriever.retrieve`, planned `mnemos/inner_life/narrative_gate.py::apply_high_blast_generated_drop`, planned `mnemos/dream_patterns/pattern_store.py::conjunctive_stabilization_gate` | U5 actual `tests/test_u5_inert_modulation.py::test_u5_rejects_evidentiary_authorities`; U6b actual `tests/test_u6b_experience_tick.py::test_tick_emits_modulation_proposal`; U6b actual `tests/test_u6b_experience_tick.py::test_reticks_are_follow_the_loop_stable`; legacy planned `tests/test_experience_tick.py::test_salience_fast_track_emits_bounded_modulation_or_proposal`; planned `tests/test_dream_graduation.py::test_authentic_recurrence_graduates_at_generated_authority` | U5 actual `tests/test_u5_inert_modulation.py::test_u5_check_constraints_hold_against_raw_sql`; U6b actual `tests/test_u6b_experience_tick.py::test_tick_refuses_identity_domain`; U6b actual `tests/test_u6b_experience_tick.py::test_tick_writes_no_modulation_row`; legacy planned `tests/test_experience_tick.py::test_high_salience_single_tick_cannot_mint_semantic_truth`; planned `tests/test_dynamic_modulation.py::test_modulation_cannot_write_belief_or_identity`; planned `tests/test_dream_graduation.py::test_stabilization_requires_all_four_conjunctive_conditions`; planned `tests/test_dream_graduation.py::test_vividness_ratio_cap_blocks_single_instance_domination`; planned `tests/test_dream_graduation.py::test_identity_domain_content_preempted_at_gate_transitions_recurring_to_decayed` |
...
| ProposalLedger row | `status=pending_review/deferred/rejected/applied`; `read_visibility=review_only/audit_only` for non-terminal proposal review, never operational for pending review | Omitted proposal `read_visibility` defaults to `audit_only`; public axes serialize as RFC `source_authority=user_stated/imported/observed/generated` and `kind=episodic/semantic/procedural/prospective`; row gains additive fields `supporting_instance_ids` (set S of supporting-instance IDs), `evidence_diversity` (computed D(P) per AM-U4, nullable = incomputable = review), and `consistency_anomaly` (computed informing score, nullable = not computed) — `evidence_diversity` participates in the AM-U4 gate decision (consumed at review/auto-apply time), while `consistency_anomaly` is an informing signal only — persisted and displayed as review context but never touching promotion eligibility or auto-apply. Neither field is a lifecycle stage; row also gains additive discriminator field `producer` (enum with values `standard` (default), `dream_graduation` (U6c pattern-store emissions)); `producer` is stamped by the caller at write time and is immutable thereafter (the discriminator is used by U6c's stricter operational-surface exception to filter dream-graduation proposals out of every operational surface). | Raw upserts on `pending_review` rows preserve or strengthen visibility (the only status raw `write_proposal` mutates); raw upserts on `deferred`, `rejected`, or `applied` rows are fail-closed (PROPOSAL_TERMINAL_STATUSES guard, unchanged from U2.5 for raw writes); `deferred` transitions to `applied` or `rejected` occur only via the U4 reviewed decision API (append-only per-decision audit records in proposal_audit_log; the deferred audit record is preserved); duplicate raw write against a deferred/rejected/applied row is rejected without mutating reason, provenance, payload, status, or visibility | `rejected` and `applied` are terminal for the proposal (immutable audit records; no further status transitions); `deferred` is a decidable intermediate status — the deferral decision's audit record is immutable, but the proposal itself can be subsequently transitioned to `applied` or `rejected` via the U4 tiered-review-gates path (standard U2.5 semantics; deferred proposals remain decidable) | create pending review/audit proposal; reviewed U4 path may later apply/reject/defer; deferred proposal may subsequently transition to `applied` or `rejected` via a new U4 decision (the deferred audit record is preserved; the new decision appends a new audit record); explicit audit/admin listing can inspect audit-only rows | pending review cannot be operational; raw write cannot forge applied/approved; individual per-decision audit records are immutable (subsequent decisions after deferral append new records rather than mutating the deferred record); status transitions on decided/deferred rows occur ONLY through the U4 reviewed decision API with append-only audit — never through raw `write_proposal` (raw write remains fail-closed on any mutation of deferred/rejected/applied rows; PROPOSAL_TERMINAL_STATUSES is unchanged from U2.5 for raw writes); `rejected` and `applied` proposals cannot re-open |
| DynamicModulation | stored/inert/backoutable; `evidentiary=false`; `recurrence_promote=false`; no identity authority | U5 stores only bounded inert modulation rows with `generated`/`observed` authority, target in `salience`/`activation`/`retrieval_weight`, magnitude <= +/-1.0, positive `ttl_seconds`, computed `expires_at`, non-empty rollout tag normalized over the shared ASCII edge-whitespace charset, optional target topic/valence, provenance, and payload; active influence omitted until U6 supplies distribution-shape bound | Recurrence may create a new event/proposal, not extend itself into durable truth or evidence; U5 does not implement recurrence extension | no active terminal state in U5; rows remain inert until explicitly deleted by rollout-tag backout or future lifecycle work | U5 store -> inert lifecycle inspection/count/backout; U6 may allow bounded influence only with TTL/decay/magnitude plus valence floor/deadband/fail-toward-width | modulation cannot cite itself as evidence, promote by recurrence, write belief/identity state, or actively shape retrieval before both bounds; a blank or padded rollout tag, nonpositive TTL, NULL/backdated expiry, evidentiary authority, or identity/recurrence flag is rejected at writer and/or schema level |
| ExperienceTick | transient afferent signal; U6b output is a ProposalLedger row targeting the modulation surface, not a stored modulation row or direct identity write | Non-identity `ProposedModulation` observations default to review-only proposal rows; identity/foundational domains are refused | Duplicate ticks update the same pending proposal ID for the same scope/family/target/topic/tag; disabled families emit nothing | none as durable memory; U6b durable effect terminates in ProposalLedger only | low-blast tick -> bounded review-visible modulation proposal; future reviewed disposal may apply through a separate U6 activation path | tick cannot directly write durable identity/foundational memory, semantic truth, or `dynamic_modulations` rows |
| Dream pattern (U6c) | `stored/recurring/stabilized/graduated/decayed`; pattern-store rows quarantined `audit_only`; recurrence count, seed-context diversity metric, instance-content diversity metric, and vividness-ratio tracked per pattern | New matched pattern increments recurrence + records instance; new-cosine-miss creates fresh pattern row with recurrence=1; conjunctive stabilization gate is off by default until all four conjunctive conditions hold (recurrences ≥ MIN, seed-context diversity, instance-content diversity, vividness-ratio cap) | Match increments recurrence and records instance without deleting prior; stabilization transitions require all four conjunctive conditions; graduation is one-time (marked graduated, no further increments); duplicate graduation attempts are rejected | `graduated` (emitted one `generated` ProposalLedger row, terminal for pattern), `decayed` (unrevisited pattern transitioned to state=decayed by the adaptive-decay window; row retained audit_only for provenance, excluded from active matching) | `stored` → `recurring` on similarity match; `recurring` → `stabilized` only when all four conjunctive conditions hold AND the narrative-gate domain check classifies content as non-identity/non-foundational; `recurring` → `decayed` if all four conjunctive conditions hold AND the narrative-gate domain check classifies content as identity/foundational (domain preempts graduation — no proposal emission, terminal, audit_only retained); `stabilized` → `graduated` emits exactly one `generated` ProposalLedger row with `read_visibility=review_only` and terminates increments; any state OTHER THAN `graduated` OR `stabilized` → `decayed` on adaptive decay (graduated is immutable terminal; stabilized is a pre-graduation state whose next step is proposal emission — adaptive decay excludes both so the required proposal emission is not preempted) | no state reaches `graduated` without the four conjunctive conditions; no re-graduation; no graduated pattern re-enters `recurring`; no path from any dream-pattern state to durable memory except through the emitted proposal's own ledger lifecycle; graduated → decayed is rejected (graduated is immutable terminal; the emitted-proposal-provenance state must survive); stabilized → decayed is rejected (adaptive decay excludes stabilized rows so the required proposal emission is not preempted); identity/foundational-domain content never reaches `stabilized` (the narrative-gate domain check is part of the stabilization gate — identity-domain rows transition directly `recurring` → `decayed`, no proposal emission, no stabilized-then-drop deadlock) |
...
| DynamicModulation lifecycle (`store_dynamic_modulation`, `get_dynamic_modulation`, `count_dynamic_modulations`, `delete_dynamic_modulations_by_rollout_tag`) | no operational read surface; rows are inert and never feed context, retrieval, salience, identity, consolidation, substrate, or MCP tools | no ordinary review surface for stored modulation rows; U6b ExperienceTick review happens through ProposalLedger rows targeting the modulation surface, not by reading modulation rows as review prose | by-ID lifecycle inspection, integer counts, and normalized-tag backout only | ranked scans, joins against memory tables, context packet inclusion, belief/proposal evidence citation, recurrence promotion, identity authority, or any read that changes retrieval/salience |
...
| Pattern store (U6c: `mnemos/dream_patterns/pattern_store.py`) | never on operational surfaces; pattern-store rows are `audit_only` and dream-pattern prose never appears as operational context | stabilized patterns surface **as ledger proposals** on review surfaces (ZERO counts, IDs, or prose on operational surfaces — U6c is STRICTER than the general proposal-metadata rule: no dream-graduation proposal count, source-ID, or provenance leaks to any operational packet, prompt, retrieval, dashboard, or aggregate; the general Surface-Ledger rule allowing count/ID exposure on other proposals is explicitly overridden for U6c); reviewer "not yet" = defer the proposal (proposal-side state remains decidable through the U4 reviewed decision API; pattern-side graduation is still terminal), "no" = reject the proposal (terminal for the proposal and terminal for the pattern; a rejected-when-weak pattern that strengthens later has no second path) | explicit audit/admin inspection of pattern-store rows only | dream-pattern prose in operational packet/prompt/retrieval; pattern-store rows seeding retrieval, graph propagation, or producer generation; re-graduation of a graduated pattern under any signal |
...
| Runtime/generated producers (`capture`, `maintain`, reflection, dream, substrate, ExperienceTick) | write review-only proposals or inert/conservative modulation according to the State and Surface ledgers; U6b ExperienceTick writes only review-visible modulation proposals | `generated` or `observed` only, as fixed by the trusted caller boundary | direct durable identity/foundational writes; self-stamping `user_stated`; bypassing ProposalLedger; ExperienceTick writing `dynamic_modulations` rows directly |
| Dream graduation (U6c pattern-store producer) | writes review-only ProposalLedger rows at `generated` authority per the four-condition conjunctive gate (recurrences ≥ MIN + seed-context diversity + instance-content diversity + vividness-ratio cap) AND only for non-identity/non-foundational domain content (the narrative-gate domain check is part of the stabilization gate; identity/foundational content is preempted `recurring` → `decayed` and never reaches emission); does not write anything else | `generated` only; no path in U6c mints anything above `generated`; caller-supplied authority is not accepted from this producer | direct durable identity/foundational writes; bypassing the four-condition conjunctive gate; per-touch stability drip; re-graduation of a graduated pattern; domain-gate exemption for stabilized identity/foundational content |
...
| Terminal ProposalLedger rows are immutable; deferred rows are raw-write immutable but proposal-side decidable only through reviewed decision API | `tests/test_store.py::TestEngramStore::test_proposal_upsert_rejects_rejected_terminal_conflict`; planned `tests/test_afferent_review_gates.py::test_reviewed_decision_api_transitions_deferred_to_applied_appending_audit_record` | `tests/test_store.py::TestEngramStore::test_proposal_upsert_rejects_applied_terminal_conflict`; planned `tests/test_store.py::TestEngramStore::test_raw_write_proposal_still_raises_on_deferred_row_mutation_proposal_terminal_statuses_unchanged` |
...
| DynamicModulation remains inert/conservative before both bounds | U5 actual `tests/test_u5_inert_modulation.py::test_u5_store_and_load_roundtrip`; U5 actual `tests/test_u5_inert_modulation.py::test_u5_g2_requires_positive_ttl`; U6b actual `tests/test_u6b_experience_tick.py::test_tick_emits_modulation_proposal`; legacy planned `tests/test_dynamic_modulation.py::test_modulation_decays_to_baseline_within_ttl` is represented in U5 by positive TTL plus `expires_at` storage, not by active decay | U5 actual `tests/test_u5_inert_modulation.py::test_u5_read_surfaces_byte_identical_with_and_without_modulations`; U5 actual `tests/test_u5_inert_modulation.py::test_u5_same_store_before_after_modulation_insert_identical`; U5 actual `tests/test_u5_inert_modulation.py::test_u5_no_module_reads_modulation_table_outside_sanctioned_files`; U6b actual `tests/test_u6b_experience_tick.py::test_tick_writes_no_modulation_row`; U6b actual `tests/test_u6b_experience_tick.py::test_batch_atomic_on_store_level_failure`; planned `tests/test_dynamic_modulation.py::test_active_influence_requires_persistence_and_distribution_bounds`; planned `tests/test_dynamic_modulation_valence_floor.py::test_uncalibrated_floor_fails_toward_width` |
...
| Identity-domain dream content is preempted at the gate (recurring -> decayed, no proposal emission) | planned `tests/test_dream_graduation.py::test_non_identity_stabilized_pattern_emits_proposal` proves non-identity stabilized patterns emit proposals | planned `tests/test_dream_graduation.py::test_identity_domain_content_preempted_at_gate_transitions_recurring_to_decayed` proves a pattern whose graduating content classifies identity/foundational at the narrative-gate domain check transitions directly recurring -> decayed (`drop:high_blast_generated`); no ProposalLedger row emitted, no stabilized-then-drop deadlock, audit_only row retained for provenance |
...
| Stage 5 | `codex/afferent-membrane-v1-modulation` / `u5-inert-storage` | U5 inert DynamicModulation persistence/backout; U6 keeps distribution bounds and active influence | U5 inertness, schema, backout, and migration tests; U6 calibration placeholder proof and no-mistakes before active influence |
| Stage 6 | `codex/afferent-membrane-v1-experience-tick` / `u6b-experience-tick` | U6b ExperienceTick proposal-only layer feeding the ProposalLedger with modulation-surface candidates | Tick-to-ledger proposal tests, inertness proof, and no-mistakes |
...
Current U5 storage state: branch `u5-inert-storage` implements only the inert
schema/write/count/backout layer. It does not satisfy the future U6
distribution-shape bound and does not authorize active retrieval influence.

Current U6b ExperienceTick state: branch `u6b-experience-tick` implements
manual, proposal-only `mnemos.modulation.ExperienceTick`. It writes review-only
ProposalLedger rows targeting the modulation surface, never writes
`dynamic_modulations`, and does not satisfy the future U6 read/apply path.
...
```mermaid
flowchart TB
  T["ExperienceTick"] --> PL["ProposalLedger<br/>modulation-surface candidates"]
  PL -. future reviewed disposal .-> DM["DynamicModulation<br/>reversible and bounded"]
  DM -. future U6 read/apply path .-> RF["Read filters<br/>TTL, magnitude, floor"]
...
- Create: `mnemos/store/proposal_audit_log.py` (append-only per-decision audit records; every reviewed decision on a proposal appends a new immutable audit row referencing the proposal_id; identity-tier decisions additionally reference the vault journal per the David-gate design; the raw proposal_ledger row status field is transitioned by the reviewed decision API only, never by raw write_proposal)
...
- Reviewed decision API (separate path from raw write_proposal). The reviewed decision API is the sole mechanism by which a proposal row status transitions from `deferred` to `applied` or `deferred` to `rejected`. Every transition appends a new per-decision audit record to proposal_audit_log; the raw proposal_ledger row status field is updated by the reviewed decision API only. Raw `write_proposal` continues to raise on any mutation of a deferred/rejected/applied row (PROPOSAL_TERMINAL_STATUSES is unchanged from U2.5 for raw writes; the fail-closed producer posture is preserved). This is the U4 realization of the David-gate design apply-path generalized to all tiers.
...
**Implementation (canonical for U4, v1.1 dedupe-then-diversity):** U4 SHIPS the v1.1 algorithm as the D(P) implementation; the strict min-pairwise definition above is the mathematical spine, and dedupe-then-diversity is the implementation that operationalizes it. Rationale: strict min-pairwise fails any set containing one near-duplicate pair even when diverse support otherwise exists ({A, A′, B, C} → non-diverse because d(A,A′) ≈ 0). Conservative direction is correct for v1, but it over-routes to review — a review-fatigue cost, and fatigue is the gate's real attack surface. v1.1: cluster near-duplicate instances first, then compute min pairwise dissimilarity across cluster representatives; a cluster contributes one representative regardless of its size. The negative test is unchanged and still binds: a pure paraphrase set collapses to a single cluster → representative count 1 → non-diverse by the degenerate-case rule. Fail-toward-review semantics unchanged.
...
- **Q5 (deferred to follow-up work) — Development-health metric set.** The exposure-ledger drift-dashboard spec is not scheduled in this plan; Q5 is dashboard-notes work that lands alongside that dashboard's implementation as a separate plan. Content (for the follow-up plan): add to the drift-dashboard as tracked series: `development_rate`, `breadth_dynamics`, `proposal_quality`, `inscribed_identity_coherence` — with causal analysis on anomaly, not threshold alarms alone. Dashboard-notes edit only; no rule text, no state; U4 does NOT ship any Q5 file, code, or test.
...
- Review-packet display: `build_context_packet` and `format_context_packet` in review mode include `evidence_diversity` and `consistency_anomaly` per pending proposal row (labels beside proposal candidate prose); operational mode omits both scores completely (no leak via counts, IDs, or labels).
- Mixed-set qualifies via dedupe-then-diversity: a supporting-instance set {A, A', B, C} where A and A' cluster as near-duplicates computes as evidence-diverse iff the cluster-representative min pairwise dissimilarity across {A(rep), B, C} clears θ_div; strict min-pairwise would reject this set for d(A, A') ≈ 0, but the canonical v1.1 dedupe-then-diversity algorithm qualifies it — the pinned test test_mixed_set_qualifies_under_dedupe_then_diversity proves the mixed-set behavior distinguishes v1.1 from strict min-pairwise.
- Migration: an existing v8 database migrates forward (v8 -> v9) by adding supporting_instance_ids, evidence_diversity, consistency_anomaly, and producer columns to the proposal_ledger table; existing rows default to empty supporting_instance_ids, NULL evidence_diversity (nullable = incomputable = review), NULL consistency_anomaly (nullable = not computed), and producer='standard'; no live-data mutation required beyond additive columns; fresh v9 databases initialize the columns with the same defaults.
- Reviewed decision happy path (deferred to applied): the reviewed decision API transitions a deferred proposal to applied via a new audit-log record; the original deferral audit record is preserved unchanged in proposal_audit_log; the proposal_ledger row status field updates to applied via the reviewed API only.
- Reviewed decision happy path (deferred to rejected): symmetric; deferred to rejected via reviewed decision API; deferral audit record preserved; proposal_ledger row status transitions to rejected via the reviewed API only.
- Negative (fail-closed raw guard preserved): raw write_proposal STILL raises "Terminal proposal rows are immutable" on ANY mutation attempt against a row whose current status is deferred, rejected, or applied; PROPOSAL_TERMINAL_STATUSES is unchanged from U2.5 for raw writes; only the reviewed decision API path may transition deferred rows. This negative is the pin that stops a future implementer from simplifying the constant.
...
**Files:**
- U5 actual modify: `mnemos/store/sqlite_store.py`
- U5 actual modify: `mnemos/store/migrations.py`
- U5 actual test: `tests/test_u5_inert_modulation.py`
- U6 planned create: `mnemos/affect/__init__.py`
- U6 planned create/modify: `mnemos/affect/dynamic_modulation.py`
- U6 planned modify: `mnemos/retrieval/reactive.py`
- U6 planned modify: `mnemos/substrate/modulators.py`
- Legacy planned test name retained for the RFC trace: `tests/test_dynamic_modulation.py`
- U6 planned test: `tests/test_retrieval.py`

**Approach:**
- Add schema v10 `dynamic_modulations` with source authority, target,
  target-topic selector, valence, magnitude, TTL, decay metadata,
  evidentiary=false, recurrence_promote=false, identity_authority=none,
  rollout tag, provenance, payload, created_at, and expires_at.
- Keep U5 inert by absence of a read path: no retrieval/salience influence, no
  context packet field, no identity-profile input, no substrate/consolidation
  input, and no MCP tool.
- Persist rows only through `store_dynamic_modulation()` and bound them at both
  writer and schema layers: `source_authority` in `generated`/`observed`,
  target in `salience`/`activation`/`retrieval_weight`, magnitude <= +/-1.0,
  positive `ttl_seconds`, `expires_at > created_at`, non-empty normalized
  `rollout_tag`, no evidence, no recurrence promotion, and no identity
  authority.
- Provide lifecycle/backout helpers only:
  `get_dynamic_modulation()` loads one row by primary key,
  `count_dynamic_modulations()` returns integer telemetry with normalized
  rollout-tag filtering, and `delete_dynamic_modulations_by_rollout_tag()`
  deletes every row for the normalized tag.
- Use a pure `CREATE TABLE IF NOT EXISTS` plus indexes for migration v10, with
  no backfill `UPDATE`, so re-running the migration on a populated database
  cannot clobber existing modulation rows.
- Define the persistence bound as TTL/decay/magnitude plus required
  rollout-tag backout and record that it is insufficient on its own for active
  retrieval steering. U6 owns any future read/apply path.
...
**Test scenarios:**
- Happy path: a modulation can be stored, loaded by ID, counted, and removed
  by rollout tag without becoming evidence.
- Happy path: every existing read surface is byte-identical with and without
  modulation rows while only U5 exists.
- Error path: modulation cannot be inserted with evidentiary authority,
  identity authority, recurrence promotion, out-of-range magnitude, nonpositive
  TTL, empty/padded rollout tag, or NULL/backdated expiry, even through raw SQL.
- Error path: no package module outside sanctioned store files references the
  `dynamic_modulations` table, and sanctioned SQL cannot perform ranked scans
  or joins against memory tables.
- Integration: migration v10 exists on fresh stores and is re-run safe on a
  populated database.

**Verification:**
- U5 tests prove inert read-surface byte identity, source-level absence of a
  consumer, sanctioned SQL shapes, fresh v10 schema/indexes, re-run-safe
  migration, store/load, required tag/TTL, raw-SQL CHECK constraints, and
  normalized tag-count/backout behavior.
- U5 does not mark active retrieval influence complete from persistence bounds
  alone; valence floor, deadband, and any read/apply path remain U6 work.
...
**Goal:** Add ExperienceTick as the afferent collection layer that emits review-visible ProposalLedger rows targeting the modulation surface, but never writes dynamic modulation rows or durable identity directly.

**Requirements boundary:** U6b is constrained by RFC-R3/R7/R8 and feeds proposal/modulation proof, but it is not a separate permission rule and does not own RFC-R3/R7/R8 independently. The implemented slice covers proposal-only tests for acceptance tests 7, 8, and 11; active modulation influence remains U6 read/apply work.

**Dependencies:** U2 proposal ledger and U5 inert modulation surface. U6b does not depend on U6 active influence because it deliberately stops at proposals.

**Files:**
- Create: `mnemos/modulation/__init__.py`
- Create: `mnemos/modulation/experience_tick.py`
- Modify: `mnemos/store/sqlite_store.py` (`MODULATION_PROPOSAL_SURFACE` and `write_proposal(commit=False)`)
- Test: `tests/test_u6b_experience_tick.py`
- Legacy planned integration names retained for later producer wiring: `tests/test_experience_tick.py`, `tests/test_simple_runtime.py`

**Approach:**
- Define `ProposedModulation` inputs around family, target, magnitude, TTL, target topic, valence, decay rate, domain, and provenance.
- Route each enabled observation into one `review_only` pending ProposalLedger row with `target_surface=MODULATION_PROPOSAL_SURFACE`; do not route into `dynamic_modulations`.
- Validate the write-contract before emission: non-empty rollout tag after `MODULATION_TAG_EDGE_WS` trimming, positive TTL, valid target, bounded magnitude, finite valence/decay, and non-negative decay.
- Refuse identity/foundational domains structurally; D7-#2 identity modulation remains unanswered and is not built.
- Use deterministic proposal IDs for `(scope, family, target, topic, rollout_tag)` so re-ticking identical input updates the same pending row instead of multiplying.
- Use `write_proposal(commit=False)` across the batch and commit once so validation or store-level failures leave no partial proposal batch.
- Keep scheduling and producer wiring out of U6b; `ExperienceTick.tick()` is manual invocation only.

**Patterns to follow:**
- Store-level proposal writing and visibility semantics in `EngramStore.write_proposal()`.
- U5 inertness tests: the U6b path may target the modulation surface as proposal data, but must not introduce a package read/write path for the `dynamic_modulations` table.

**Test scenarios:**
- Happy path: a valid observation emits one `review_only` pending modulation proposal with rollout tag and TTL in the payload.
- Happy path: re-ticking identical input updates the same proposal ID; distinct family/target/topic/tag combinations emit distinct proposals.
- Error path: invalid rollout tag, target, TTL, identity/foundational domain, valence, decay rate, or store-domain enum failure emits no partial batch.
- Error path: disabled families emit nothing.
- Inertness: the tick writes no `dynamic_modulations` row and adds no read/apply path.

**Verification:**
- `tests/test_u6b_experience_tick.py` proves ExperienceTick cannot bypass the proposal ledger, cannot write identity/foundational-domain proposals, cannot write modulation rows, and cannot multiply or partially persist proposal batches.
...
- **Graduation — one-time, non-accumulating.** A stabilized pattern graduates exactly once: it emits one ProposalLedger row at `generated` authority and is marked graduated — no further increments, no re-emission, no per-touch stability drip. The emitted row also carries `producer='dream_graduation'` (the discriminator U6c's stricter operational-surface exception filters on; the producer field is caller-stamped at write time and immutable thereafter). The design keyword is binding: non-accumulating. Authority routing for dream graduation is STRICTER than the general `generated`-authority default: dream-graduation proposals emit `read_visibility=review_only` unconditionally across all tiers (including episodic-non-identity), because generative dream content requires reviewer visibility before it can influence any operational surface. The membrane's general `generated`-authority auto-apply in the episodic-non-identity tier is EXPLICITLY overridden for U6c. Domain gate on graduating content is unconditional and preemptive: the narrative-gate domain check is part of the stabilization gate — identity/foundational-domain dream content is preempted at the gate (`drop:high_blast_generated`) and the pattern transitions directly to `decayed` (terminal, audit_only retained for provenance, NO ProposalLedger row emitted). Non-identity domains transition to `stabilized` and then graduate; identity/foundational domains never reach `stabilized`. Stability earns a proposal only for non-identity content; a domain exemption is impossible by construction.
...
**Test scenarios:**
- Happy path: authentic recurrence graduates as a proposal — a pattern recurring ≥ MIN across seed-diverse, pairwise-dissimilar instances within the vividness cap emits exactly one `generated` ProposalLedger row with `read_visibility=review_only` (visible on the review queue via `mnemos_review_queue`, quarantined from every operational surface — zero counts, IDs, or prose leak to any operational surface (STRICTER than the general Surface-Ledger proposal-metadata rule)).
...
- Negative path: identity-domain dream content is preempted at the gate — a pattern whose graduating content classifies identity/foundational at the narrative-gate domain check transitions directly `recurring` → `decayed` (`drop:high_blast_generated`, no proposal row, no private write, no stabilized-then-drop deadlock, audit_only row retained for provenance).
...
- Immutable graduation: a graduated pattern is excluded from adaptive decay; graduated rows persist audit_only in-place for the emitted-proposal-provenance lifetime; no adaptive-decay scheduler may transition a graduated row to state=decayed.
- Atomic stabilize-to-graduate: a stabilized pattern is excluded from adaptive decay until it emits its ProposalLedger row; no adaptive-decay scheduler may transition a stabilized row to state=decayed; the required proposal emission is never preempted.
- Operational-surface exception: for U6c-emitted ProposalLedger rows, `build_context_packet` in operational mode surfaces ZERO counts, ZERO source IDs, and ZERO prose (assertion: dream-graduation proposal counts are excluded from operational proposal counts; source IDs are excluded from operational ID lists; `format_context_packet` and visual/dashboard surfaces produce no dream-graduation row identifiers); the STRICTER-than-general Surface-Ledger rule is enforced by explicit filter on `producer='dream_graduation'`, not by convention: context_packet, visual_snapshot, mcp_server, and store aggregate counts explicitly exclude rows where `producer='dream_graduation'` from every operational surface (counts, IDs, prose, review-count aggregates).
- Migration + defaults: an existing v9 (or later) database migrates forward to add the dream pattern_store table and the dream-pattern lifecycle columns (stored/recurring/stabilized/graduated/decayed with recurrence count, seed-context diversity metric, instance-content diversity metric, and vividness-ratio); fresh initialization creates the pattern_store empty; every row remains present in all lifecycle states including decayed (audit_only quarantine is the retention mechanism; there is no physical purge path); every row defaults to read_visibility=audit_only (dream-pattern prose never operational); no PAI/live-data mutation is required beyond additive schema.

**Review-surface documentation note (binding):** the "maybe later" path for a graduated pattern is proposal deferral, never re-graduation. Graduation is one-time (Approach above) and a graduated pattern never re-enters `recurring` (State Ledger) — so a pattern's emitted proposal is its only question, ever. A reviewer who means "not yet" must defer the proposal, and a reviewer who means "no" must reject it. **Pattern-side (graduation lifecycle):** both dispositions are terminal for the pattern — graduation is one-time by charter, and re-graduation is forbidden regardless of the proposal's fate. **Proposal-side (standard U2.5 semantics):** `deferred` proposals remain decidable — the deferral audit record is immutable, but a reviewer may subsequently apply or reject the proposal via the U4 tiered-review-gates path; `rejected` proposals are terminal for the proposal. The two lifecycles are independent: pattern-side one-time-ness holds regardless of proposal outcome, and proposal-side deferral stays open in the standard U2.5 sense. A rejected-when-weak pattern that genuinely strengthens later has no second path via new graduation (pattern cannot ask twice), but the reviewer's honest "not yet" preserves proposal-decidability without foreclosing later application. (Aligns with rejection-lineage anti-fatigue and Fable's Amendment B review pinning of the "maybe later" path.)
...
| 7. DynamicModulation cannot be cited as evidence | U5/U6b | `tests/test_u5_inert_modulation.py::test_u5_read_surfaces_byte_identical_with_and_without_modulations`; `tests/test_u5_inert_modulation.py::test_u5_rejects_evidentiary_authorities`; `tests/test_u6b_experience_tick.py::test_tick_proposals_are_review_visible`; legacy planned `tests/test_dynamic_modulation.py::test_dynamic_modulation_cannot_be_cited_as_evidence` remains the higher-level U6/ExperienceTick phrasing |
| 8. DynamicModulation cannot promote by recurrence | U5/U6b | `tests/test_u5_inert_modulation.py::test_u5_check_constraints_hold_against_raw_sql`; `tests/test_u5_inert_modulation.py::test_u5_no_module_reads_modulation_table_outside_sanctioned_files`; `tests/test_u6b_experience_tick.py::test_reticks_are_follow_the_loop_stable`; legacy planned `tests/test_dynamic_modulation.py::test_dynamic_modulation_cannot_promote_by_recurrence` remains the higher-level U6/ExperienceTick phrasing |
| 9. DynamicModulation decays to baseline within TTL | U5 | `tests/test_u5_inert_modulation.py::test_u5_g2_requires_positive_ttl`; `tests/test_u5_inert_modulation.py::test_u5_g4_check_pins_expiry_against_raw_sql`; legacy planned `tests/test_dynamic_modulation.py::test_modulation_decays_to_baseline_within_ttl` maps to U5's stored TTL/expiry bound only, not active decay |
...
| 11. DynamicModulation cannot effect belief/identity write | U5/U6b/U6 | `tests/test_u5_inert_modulation.py::test_u5_same_store_before_after_modulation_insert_identical`; `tests/test_u5_inert_modulation.py::test_u5_sanctioned_files_contain_no_ranking_read_of_modulations`; `tests/test_u6b_experience_tick.py::test_tick_writes_no_modulation_row`; `tests/test_u6b_experience_tick.py::test_tick_refuses_identity_domain`; planned `tests/test_dynamic_modulation.py::test_modulation_cannot_write_belief_or_identity`; legacy planned `tests/test_experience_tick.py::test_high_salience_single_tick_cannot_mint_semantic_truth` |
...
| DynamicModulation becomes hidden durable memory | Medium | High | U5 requires TTL/decay/expiry, non-evidentiary flags, no recurrence promotion, required normalized rollout-tag backout, and proof that no read path exists. |
...
- U5 documentation now covers inert `dynamic_modulations` storage, schema v10
  constraints, required rollout-tag backout, positive TTL/expiry, normalized
  tag counting/deletion, and the absence of retrieval/salience/context/identity
  read paths in README, changelog, architecture, privacy/security,
  release-hardening, and turnkey-agent docs.
- U6b documentation now covers proposal-only `ExperienceTick`, review-visible
  modulation-surface proposal rows, deterministic re-tick IDs, per-family
  kill-switches, rollout-tag/TTL/numeric validation, identity-domain refusal,
  store-level batch rollback, and the absence of `dynamic_modulations` row
  writes or read/apply paths in README, changelog, architecture,
  privacy/security, release-hardening, turnkey-agent docs, and this plan.
